	...
	if np.shape(current)[0] == 1000000:
		...
		if np.amin(current[:,4]) > -1:
			...
			if training_file == True:
				...
				neighbors_4D_aux = NearestNeighbors(n_neighbors=10, algorithm='ball_tree').fit(fit_to_this)

				current = np.take(current,np.random.permutation(current.shape[0]),axis=0,out=current)
				columns_to_fit_to = [indexes_ptparam['r']]
				fit_to_this = current[:num_points,columns_to_fit_to]
				neighbors_r_aux = NearestNeighbors(n_neighbors=10, algorithm='ball_tree').fit(fit_to_this)

				current = np.take(current,np.random.permutation(current.shape[0]),axis=0,out=current)
				columns_to_fit_to = [indexes_ptparam['z']]
				fit_to_this = current[:num_points,columns_to_fit_to]
				neighbors_z_aux = NearestNeighbors(n_neighbors=10, algorithm='ball_tree').fit(fit_to_this)

				current = np.take(current,np.random.permutation(current.shape[0]),axis=0,out=current)
				columns_to_fit_to = [indexes_ptparam['pt']]
				fit_to_this = current[:num_points,columns_to_fit_to]
				neighbors_pt_aux = NearestNeighbors(n_neighbors=10, algorithm='ball_tree').fit(fit_to_this)

				current = np.take(current,np.random.permutation(current.shape[0]),axis=0,out=current)
				columns_to_fit_to = [indexes_ptparam['pz']]
				fit_to_this = current[:num_points,columns_to_fit_to]
				neighbors_pz_aux = NearestNeighbors(n_neighbors=10, algorithm='ball_tree').fit(fit_to_this)
			...
			if plotting_example == True:
				...
				for aux_id_i in range(0,5):
					...
					plt.savefig('/Users/am13743/Aux_GAN_thesis/THESIS_ITERATION/DATA/TEST_PTPARAM_aux_values_%d.pdf'%aux_id_i,bbox_inches='tight')
					plt.close('all')
			...
			if training_file == False:
				print('Saving file...')
				np.save('/Users/am13743/Aux_GAN_thesis/THESIS_ITERATION/DATA/NEW_TEST_%d'%file_id, current)
				quit()
				...
				for aux_id_i in range(0,5):
					...
					for i in range(0, 6):
						for j in range(i+1, 6):
							...
							plt.ylabel(axis_titles[j])
					plt.subplots_adjust(hspace=0.3,wspace=0.3)
					plt.savefig('/Users/am13743/Aux_GAN_thesis/THESIS_ITERATION/DATA/TEST_QTBOXCOX_aux_values_%d.pdf'%aux_id_i,bbox_inches='tight')
					plt.close('all')
				...
				for aux_id_i in range(0,5):
					...
					for i in range(0, 6):
						for j in range(i+1, 6):
							...
							plt.ylabel(axis_titles[j])
					plt.subplots_adjust(hspace=0.3,wspace=0.3)
					plt.savefig('/Users/am13743/Aux_GAN_thesis/THESIS_ITERATION/DATA/TEST_Norm_physical_aux_values_%d.pdf'%aux_id_i,bbox_inches='tight')
					plt.close('all')
