...
auxiliary_inputs = Input(shape=(1,1))
...
discriminator_aux_pt.load_weights('/%s/D_AUX_%s_WEIGHTS.h5'%(pre_trained_directory,aux_name))
#############################################################################################################
...
def train_step(images, weight_of_reco_loss_inner):
	...
	images, aux_values = images[:,:,:7], images[:,:,index_of_aux_column] # Just P_t aux

	noise = tf.random.normal([batch_size, 1, 100])
	aux = tf.math.abs(tf.random.normal([batch_size, 1, 1]))
	...
	aux_stacked = tf.math.abs(tf.random.normal([batch_size, 1, 1]))
	...
	with tf.GradientTape(persistent=True) as gen_tape:
		...
		stacked_output_reco = discriminator_aux_pt(fake_images2)
		gen_loss_GAN = _loss_generator(tf.squeeze(labels_stacked),tf.squeeze(stacked_output_choice))
		gen_loss_reco = tf.keras.losses.mean_squared_error(tf.squeeze(aux_stacked),tf.squeeze(stacked_output_reco))
		gen_loss = gen_loss_reco*weight_of_reco_loss_inner + gen_loss_GAN*weight_of_vanilla_loss
